...
import { useAddRider } from "@/src/hooks/useRiders";
...
interface QuickAddRiderSheetProps {
  open: boolean;
...
export default function QuickAddRiderSheet({
  open,
  sellerId,
  onClose,
  onSaved,
}: QuickAddRiderSheetProps) {
  const { colors, isDark } = useTheme();
  const insets = useSafeAreaInsets();
  const showToast = useToastStore((state) => state.show);
  const addRider = useAddRider(sellerId);
  const snapPoints = useMemo(() => [480 + insets.bottom], [insets.bottom]);
  ...
  const handleSave = handleSubmit(async (values) => {
    try {
      const rider = await addRider.mutateAsync({
        name: values.name,
        phone: values.phone,
      ...
      showToast("Couldn't save rider. Try again.", "error");
    }
  });
  ...
  return (
    ...
    >
      ...
      >
        <Text style={[styles.title, { color: colors.textPrimary }]}>Add Rider</Text>
