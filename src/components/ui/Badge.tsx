 ...
 * Supports all 5 order statuses + custom fg/bg override.
 *
 * Usage:
 *   <Badge status="in_transit" />
 *   <Badge status="picked_up" />
 *   <Badge label="Custom" fg={colors.warning} bg={colors.warningBg} />
 *
...
function buildStatusMap(colors: ReturnType<typeof useTheme>["colors"]) {
  return {
    ...
    picked_up:  { label: "Picked Up",  fg: colors.info,    bg: colors.infoBg    },
    in_transit: { label: "In Transit", fg: colors.info,    bg: colors.infoBg    },
    delivered:  { label: "Delivered",  fg: colors.success, bg: colors.successBg },
    failed:     { label: "Failed",     fg: colors.error,   bg: colors.errorBg   },
  } as const;
}
...
export function Badge({ status, label, fg, bg, style }: BadgeProps) {
  ...
  if (status) {
    ...
    displayLabel = label ?? map[status].label;
  } else {
